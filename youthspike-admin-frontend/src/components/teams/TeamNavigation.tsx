  ...
  return (
    <div className="px-3 py-2">
      <div className="flex gap-2 bg-gray-700 rounded-lg p-1 mb-2">
        <NavLink href={`/${eventId}/teamstandings/${ldoIdUrl}`} isActive={false}>
          Standings
        </NavLink>
...
const NavLink = ({ href, children, isActive }: { href: string; children: React.ReactNode; isActive: boolean }) => (
  <Link
    href={href}
    className={`uppercase flex-1 py-2 px-2 rounded-md text-xs font-bold transition-all text-center ${isActive ? 'bg-yellow-400 text-gray-900 shadow-sm' : 'text-gray-300 hover:text-white bg-gray-700'}`}
  >
    {children}
  </Link>
);
