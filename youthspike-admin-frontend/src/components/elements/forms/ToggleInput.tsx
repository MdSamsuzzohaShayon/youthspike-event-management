...
const ToggleInput = (props: IToggleInputProps) => {
  ...
  return (
    <div className={`input-item mt-4 ${props.extraCls} ${props.widthCls ? props.widthCls : 'w-full'} flex justify-between items-center`}>
      ...
      <div className={`border border-yellow-500 ${dv === true ? 'bg-yellow-500' : 'bg-transparent'} outline-none px-2 rounded-full h-10 w-20 text-center flex items-center justify-between`}>
        {dv && <span>Off</span>}
