...
const ToggleInput = (props: IToggleInputProps) => {
  ...
  return (
    <div className={`input-item mt-4 ${props.extraCls} ${props.widthCls ? props.widthCls : 'w-full'} flex justify-between items-center`}>
      ...
      <div className={`border border-yellow-logo ${dv === true ? 'bg-yellow-logo' : 'bg-transparent'} outline-none px-2 rounded-full h-10 w-20 text-center flex items-center justify-between`}>
        {dv && <span>Off</span>}
        <button type="button" className='w-7 h-7 bg-white rounded-full' onClick={e=>props.handleValueChange(e, props.name)} ></button>
        {!dv && <span>On</span>}
      </div>
    </div>
  )
