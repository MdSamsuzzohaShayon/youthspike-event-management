    ...
    return (
        <div className={`input-group mt-4 w-full flex ${props.vertical ? 'flex-col' : ''} justify-between items-center ${props.extraCls}`}>
            ...
            <input onChange={props.handleInputChange}
                ...
                className={`${props.vertical ? 'w-full' : ''} border border-gray-300 bg-transparent outline-none px-2 rounded-lg h-10 text-center datetime-input ${props.rw ? props.rw : "w-20"}`} type='date' required={props.required} />
        </div>
    )
}
