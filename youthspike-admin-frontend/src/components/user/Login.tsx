  ...
  return (
    <div className="flex w-full min-h-screen bg-yellow-gradient">
      <div className="flex flex-col items-center justify-center w-full md:w-1/2 px-6 py-12 md:py-0">
        <motion.h1 className="text-4xl font-bold text-white mb-6" initial={hInitial} animate={hAnimate} exit={hExit}>
          Login
        </motion.h1>
        <div className="w-full md:w-4/6">
          <motion.div initial={cInitial} animate={cAnimate} exit={cExit} className="flex justify-center mb-6">
            <Image alt="Logo" src="/free-logo.png" width={80} height={80} className="rounded-full shadow-md" />
          </motion.div>
          <form onSubmit={handleLogin} className="flex flex-col gap-4 bg-gray-900 p-8 rounded-lg shadow-lg bg-gray-gradient-linear">
            ...
            <motion.button type="submit" className="w-full btn-info" initial={cInitial} animate={cAnimate} exit={cExit}>
              Submit
            </motion.button>
            {!passcodeOpener && (
              <p role="presentation" onClick={(e) => setPasscodeOpener(true)}>
                Have passcode?
              </p>
            )}
          </form>
