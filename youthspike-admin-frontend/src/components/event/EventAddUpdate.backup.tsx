...
import { IProStats, IProStatsAdd } from '@/types/playerStats';
import addOrUpdateEvent from '@/utils/requestHandlers/addOrUpdateEvent.backup';
...
function EventAddUpdate({ update, prevEvent }: IEventAddProps) {
  ...
  const handleProStatsInputChange = (e: React.SyntheticEvent) => {
    const inputEl = e.target as HTMLInputElement;
    const [namePrefix, fieldName] = inputEl.name.split('.') as [
      'multiplayer' | 'weight' | 'stats',
      keyof IProStats
    ];
    const value = parseFloat(inputEl.value);
  
    // Update the corresponding state based on the namePrefix
    switch (namePrefix) {
      case 'multiplayer':
        setMultiplayer(prev => ({
          ...prev,
      ...
      case 'stats':
        ...
        break;
      default:
        console.warn(`Unknown pro stats prefix: ${namePrefix}`);
    }
  };
  ...
  return (
    <form className="w-full" onSubmit={handleEventAdd}>
      ...
      <div className="w-full pro-stats mt-4">
        <ProStatsInput label="Multiplayer Stats" namePrefix="multiplayer" defaultValue={multiplayer || {}} handleInputChange={handleProStatsInputChange} />

        <ProStatsInput label="Weight Stats" namePrefix="weight" defaultValue={weight || {}} handleInputChange={handleProStatsInputChange} />

        <ProStatsInput label="Stat Stats" namePrefix="stat" defaultValue={stats || {}} handleInputChange={handleProStatsInputChange} />
      </div>
