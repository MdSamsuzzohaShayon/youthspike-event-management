...
const SRPlayerCard: React.FC<ISPPlayerCardProps> = ({ selected, serverReceiverTeam, player, role, dark, handlePlayerSelection }) => {
  ...
  const teamOfPlayer = useMemo(() => {
    ...
    if (teamA?._id && player.teams?.some((team) => (typeof team === 'string' ? team === teamA._id : team._id === teamA._id))) return teamA;
    if (teamB?._id && player.teams?.some((team) => (typeof team === 'string' ? team === teamB._id : team._id === teamB._id))) return teamB;
    return null;
  }, [player, teamA, teamB]);
    ...
    >
      ...
      <button
        ...
        className={`group w-24 h-24 lg:w-32 lg:h-32 rounded-2xl border-4 overflow-hidden
                    ...
                    ${selected ? 'border-yellow-400 bg-gray-800 shadow-inner' : dark ? 'border-gray-700 bg-gray-700' : 'border-yellow-400 bg-gray-800'}`}
      >
        {selected && serverReceiverTeam ? (
          player?.profile ? (
            <CldImage alt={`${player.firstName} ${player.lastName}`} width="200" height="200" className="object-cover w-full h-full" src={player.profile} />
          ) : (
      ...
      {selected && serverReceiverTeam && (
        <div className="mt-3 text-center">
          ...
          <p className={`text-xs sm:text-sm md:text-base font-medium break-words max-w-[100px] sm:max-w-[150px] md:max-w-[200px] w-full ${teamTextColor}`} title={teamOfPlayer?.name ?? ''}>
            {teamOfPlayer?.name}
          </p>
        </div>
      )}
