...
function PublicMenu({ accessCodeList }: IPublicMenuProps) {
  // ===== Hooks =====
  ...
  const handleResetAccessCode = (e: React.SyntheticEvent) => {
    e.preventDefault();
      ...
      {isMenuOpen && (
        ...
        >
          ...
          <div className="mt-8 flex flex-wrap justify-start items-center gap-2">
            ...
            {accessCodeList && accessCodeList.length > 0 && (
              <button className="btn-danger" onClick={handleResetAccessCode}>
                Reset Access Code
              </button>
            )}
          </div>
