    ...
    return (
        <div className="flex justify-start items-center">
            <div className="logo m-2">
                {team.captain?.profile ? <AdvancedImage cldImg={cld.image(team.captain?.profile)} className="w-12" alt={team.captain?.firstName} /> : <img src='/free-logo.svg' className="w-16" alt='free-logo' />}
            </div>
            <div className="detail m-2 ">
                <h3>{home ? 'Home': 'Invited'} Team: {team.name}</h3>
