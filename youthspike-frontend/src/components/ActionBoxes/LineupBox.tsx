...
function LineupBox({ currRoom, user, socket, otp, mtp }: IBoxProps) {
  ...
  const handleChangeRound = async (e: React.SyntheticEvent, next: boolean) => {
    ...
    if (newRoundIndex !== -1) {
      // changeTheRound({ socket, roundList, currRound: currentRound, dispatch, allNets, currRoom, newRoundIndex, myTeamE, opTeamProcess })
    }
  }
    ...
    <div>
      {mtp === EActionProcess.LINEUP ? <div>
        ...
        <div className="buttons flex w-full justify-center items-center gap-x-2">
          ...
          <button className="btn-primary" type='button' onClick={(e) => handleChangeRound(e, true)}>Next Round</button>
        </div>
