  ...
  return (
    <>
      ...
      <div className={`p-name-rank w-full flex ${screenWidth > screen.xs ? 'h-7' : 'h-9'}`}>
        <div className="rank w-4 h-full bg-yellow-400 text-black-logo text-lg flex justify-center items-center" style={fsToggle(screenWidth)}>
          {player ? player.rank : 0}
        </div>
        <p className={`name flex justify-center items-center w-12 leading-3 capitalize break-words ${dark ? 'text-white' : 'text-black-logo'}`} style={fsToggle(screenWidth)}>
