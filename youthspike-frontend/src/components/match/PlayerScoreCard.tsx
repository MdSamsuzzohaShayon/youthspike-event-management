  ...
  return (
    <React.Fragment>
      <div className={`p-img-wrap relative w-full ${screenWidth > screen.xs ? 'h-20' : 'h-24'}`}>
        {shouldShowEvacuateButton && (
          <div className="absolute top-1 right-1 w-4 bg-gray-900 rounded-full">
            {myTeamE === ETeam.teamA ? (!currentRound.teamAScore && (
        ...
        {['left', 'right'].map((position) => (
          <div key={position} className={`img-${position}-txt absolute bg-slate-100/75 text-gray-900 bottom-0 ${position}-0 w-7 text-center flex flex-col`} style={fsToggle(screenWidth)}>
            <span>1-1</span>
      ...
      <div className={`p-name-rank w-full flex ${screenWidth > screen.xs ? 'h-7' : 'h-9'}`}>
        <div className={`rank w-4 h-full bg-yellow-500 text-gray-100 text-lg flex justify-center text-center items-center`} style={fsToggle(screenWidth)}>
          {player ? player.rank : 0}
        </div>
        <p className={`name flex justify-center text-center items-center w-12 leading-3 capitalize break-words ${dark ? 'text-gray-100' : 'text-gray-900'}`} style={fsToggle(screenWidth)}>
