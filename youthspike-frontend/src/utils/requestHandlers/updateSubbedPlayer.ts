...
import { IRoundRelatives, TMutation } from "@/types";
...
interface IRemovePlayerProps {
  ...
  mutateRound: TMutation;
  roundList: IRoundRelatives[];
}

const updateSubbedPlayer = async ({
  playerId,
  currRound,
  mutateRound,
  dispatch,
  ...
  try {
    ...
    const subPlayerIds: string[] = [
      ...roundObj.subs.filter((s) => s !== playerId),
    ];
    ...
    await mutateRound({
      variables: {
        updateInput: {
          subs: subPlayerIds,
          roundId: currRound._id,
          matchId: currRound.match,
        },
      },
    });
    for (let rIdx = 0; rIdx < roundList.length; rIdx += 1) {
      ...
      if (
        roundObjUpdate.match === roundObj.match &&
        roundObjUpdate.num >= roundObj.num
      ) {
