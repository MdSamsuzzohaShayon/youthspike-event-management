  ...
  for (let i = 0; i < currRoundNets.length; i++) {
    ...
    if (currMatch.netVariance) {
      ...
      if (pairScore > maxPairScore && matchUp) {
        ...
        while (limit) {
          const tempRp1: null | undefined | IPlayer = availablePlayers[mI + 1];
          const tempRp2: null | undefined | IPlayer = availablePlayers.find((p) => p.rank && tempRp1?.rank && p.rank + tempRp1?.rank <= maxPairScore);
        ...
        const limit = Math.ceil(availablePlayers.length / 2);
        while (limit) {
          const tempRp1: null | IPlayer = availablePlayers[mI + 1];
          const tempRp2: null | IPlayer = availablePlayers[availablePlayers.length - (mI + 1)];
          const nps = (tempRp1.rank || 0) + (tempRp2.rank || 0);
          if (nps >= maxPairScore) {
            rp1 = tempRp1;
            rp2 = tempRp2;
