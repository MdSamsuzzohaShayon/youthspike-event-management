...
This is sometime working, sometime not. 
SO I am giving you instructions and you will make the logic again. 

## Rules:
1. We will basically select players for each net of allNets. FOr each net we need to set 2 player, if we are in teamA then we need to set teamAPlayerA and teamAPlayerB. On the other hand, if we are in team B then we need to set teamBPlayerA and teamBPlayerB.
This is type reference for allNets array:
...
2. We need to select players from mySortedPlayers totally randomly. 
this is type reference of player from mySortedPlayers array:
...
```
3. One player can not be setted in multiple nets


4. In some cases we can have 3 nets, but 5 players or 7 players. if we have 5 players then we will set 2x2 =4 players in first 2 nets, and in the last net we will set 1 players. If we have 7 players we will sset 2x3=6 players randomly in all 3 nets and remaining players will do nothing.


5. A player can not be pair with his same partner in 2 consecutive round. Check current round num, if round num is 1 that means that was no previous round, so skip the action. Then if the round num is more than one, then check if selected player has been in the a net in previous round with same partner, then change the partner. if one round has passed then a player can be in the same round . that means 2 same player can not be in a net in 2 nest to next round, if one round gap then both can play again in the same net. roundList is an array of all round.

Current round type reference:
...
6. We can have `matchUp`, `matchUp` means oponent team has submitted their lineup for each net, now we need to submit. If we have matchUp then we  need to check oponent's pair score. pair score will be calculated by adding 2 players rank number. Then we need to check oour pair score. then we need to netVariance. For examle, we are assigning in net 2, for this specific net we need to check the oponent team, oponent team set teamAPlayerA, and teamAPlayerB by adding both of the player's rank we got 7, and our netVariance is 3, so we need to set 2 players in teamBPlayerA and teamBPlayerB, when we set those 2 player their pair score must be within 4 to 10. Because net netVariance 3 and oponent pair score 7, in total 10, and sub stracting 3 from 7 is 4.
8. Limits players to top 3 for extended overtime

## Coding style
1. Use clear, meaningful, and self-explanatory names for all variables and functions.
2. Apply the KISS (Keep It Simple) and DRY (Don’t Repeat Yourself) principles to ensure the code is clean, readable, and maintainable.
3. Add proper TypeScript typings to improve type safety and reduce potential runtime errors. Try not to use any type
4. Optimize the implementation for better time and space complexity, ensuring improved performance and scalability where possible.
5. Identify and fix any existing errors so the code runs correctly, without altering the original business logic.
6. Break down complex logic into smaller helper functions or subcomponents where it improves readability and structure. Try to make pure functions if possible. Must Apply Single Responsibility Principle (SRP)
7. Implement robust and clean error handling, ensuring edge cases are handled gracefully.


Note: if anything unusual then throw an error