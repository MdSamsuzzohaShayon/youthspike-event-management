...
import {
    mockLdoService, mockTeamService, mockSponsorService,
    mockPlayerService, mockMatchService, mockGroupService,
...
describe('EventFields', () => {
    ...
    describe('ldo', () => {
        it('calls ldoService.findByDirectorId with event.ldo as string', async () => {
            ...
            ldoService.findByDirectorId.mockResolvedValueOnce({ _id: 'ldo-1', name: 'LDO' });
    ...
    describe('groups', () => {
        ...
        it('throws "Failed to fetch event groups" when groupService.find throws', async () => {
            ...
            await expect(fields.groups(event as any)).rejects.toThrow('Failed to fetch event groups');
            expect(spy).toHaveBeenCalled();
            spy.mockRestore();
        });
    });
