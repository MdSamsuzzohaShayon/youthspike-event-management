...
import { EPlayStrategy, ReceivingHittingErrorInput } from '../gateway.types';
...
export class ReceivingHittingErrorHandler {
  ...
  async handle(@ConnectedSocket() client: Socket, @MessageBody() body: ReceivingHittingErrorInput, server: Server) {
    try {
      // Serving team Scores
      ...
      const receiverUpdatedKeys = this.scoreKeeperHelper.increment(
        stats[net.receiver as string],
        receivingStats.receiver,
      );

      const receivingPartnerUpdatedKeys = this.scoreKeeperHelper.increment(
        stats[net.receivingPartner as string],
        receivingStats.receivingPartner,
      );
      ...
      await Promise.all([
        ...
        this.scoreKeeperHelper.publishRoom(body.room, 'receiving-hitting-error-from-server', {
