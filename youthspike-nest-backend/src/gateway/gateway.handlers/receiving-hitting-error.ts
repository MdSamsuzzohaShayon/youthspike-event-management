...
import { RoomLocal, ServerReceiverOnNet, ReceivingHittingErrorInput } from '../gateway.types';
...
export class ReceivingHittingErrorHandler {
  ...
  async handle(
    ...
    try {
      ...
      const NET_CACHE_KEY = `sr:${receivingHittingErrorInput.net}:${receivingHittingErrorInput.room}`;

      // player service opportunity increase
      const netCacheData = await this.gatewayRedisService.getAction(NET_CACHE_KEY);
      if (!netCacheData) {
        throw new Error(`Net data not found, try setting players on the net again: ${receivingHittingErrorInput}`);
      }
      ...
      const netExist = await netService.findById(receivingHittingErrorInput.net);
      ...
      receiverStatsData.receivedCount = receiverStatsData.receivedCount + 1;
      ...
      await this.gatewayRedisService.setAction(RECEIVER_PLAYER_CACHE, receiverStatsData);
       ...
       // Receiver Partner / Setter
       const RECEIVER_PARTNER_PLAYER_CACHE = `player:${actionData.receiver}`;
      ...
      if (!serverInTeamA && !serverInTeamB) {
        throw new Error(`Server is not part of the net. Input: ${receivingHittingErrorInput}`);
      }
      ...
      await this.gatewayRedisService.publishToRoom(
        receivingHittingErrorInput.room,
        'receiving-hitting-error-from-server',
        actionData,
        // client.id,
      );
    } catch (error: any) {
