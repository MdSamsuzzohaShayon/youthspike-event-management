...
import { EPlayStrategy, RallyConversionInput } from '../gateway.types';
...
export class RallyConversionHandler {
  ...
  async handle(@ConnectedSocket() client: Socket, @MessageBody() body: RallyConversionInput, server: Server) {
    try {
      ...
      await Promise.all([
        ...
        this.scoreKeeperHelper.publishRoom(body.room, 'rally-conversion-from-server', {
